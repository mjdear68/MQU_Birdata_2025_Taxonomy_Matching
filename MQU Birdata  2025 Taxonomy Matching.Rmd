...
j1_match <- j1 |> filter(!is.na(Scientific_Name_ALA)) # Matched records
j1_resid <- j1 |> filter(is.na(Scientific_Name_ALA)) # Unmatched records; pass to next level
```

```{r}
BD_species |> mutate(across(everything(), str_to_lower)) |> head()
```


```{r join 2}
j2 <- left_join(join_sci_name, ALA_species, by=c("Common_Name.x", "Common_name"), keep = TRUE)
```
