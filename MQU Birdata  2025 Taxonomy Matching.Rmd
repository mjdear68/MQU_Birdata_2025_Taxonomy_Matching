...
Species names for a KBA can be obtained from BD (<https://birdata.birdlife.org.au/>) by following these steps:
...
Species names for a KBA can be obtained from ALA (<https://ala.org.au/>) by following these steps: 

* Open the 'Spatial Analysis' from the 'Search and Analyse' menu at the top of the home page 
...
# Usage Example
The following code will merge the "Scientific_Name_ALA" and "Common_Name_ALA" from the translation table (`df_trans`) into the Birdata dataset (`BD`).

```{r merge ALA names}
# Uncomment and modify to suit the Birdata dataset
# BD_merge <- left_join(BD, df_trans |> select(Scientific_Name_BD, Scientific_Name_ALA, Common_Name_ALA),
#                 by = c(Scientific_Name, Scientific_Name_BD) # Scientific_Name is the key from BD
#                 )
```
