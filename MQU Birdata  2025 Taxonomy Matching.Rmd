...
readDIR <- function(path, 
  ...
  # Read the directory of files
	...
	# Combine to form final data frame
	df <- readr::read_delim(list_of_files, 
		id = "file_name", 
		skip = skip, 
		delim = delim, 
		col_select = cols,
...
readDIR <- function(path, 
  ...
  # Read the directory of files
	...
	# Fix column names
	out <- out |>
	  rename_with(function(n){str_replace_all(n, str_replace_pattern)})
...
# BD_species <- readDIR(path = "./Input/BD/", cols=1:3, str_replace_pattern = c("[\\s-]" = "_"))
# ALA_species <- readDIR(path = "./Input/ALA/", cols=c("Species Name", "Vernacular Name"))
BD_species <- readDIR(path="./Input/BD/", cols=c(1:2)) |> relocate(2,1)

ALA_species <- readDIR(path="./Input/ALA/", cols=c("Species Name", "Vernacular Name")) |> rename("Scientific_Name"="Species_Name", "Common_Name"="Vernacular_Name")
```
